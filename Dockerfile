...
RUN apk add curl jq
RUN wget $(curl -s https://api.github.com/repos/tgbot-collection/YYeTsFE/releases/tags/ads-2026-04-28-redirect | jq -r '.assets[] | select(.name == "build.zip") | .browser_download_url')
